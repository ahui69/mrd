...
def _replicate_generate(prompt: str, size: str, user: str) -> str | None:
    ...
    try:
        r = requests.post(
            REPLICATE_BASE, json=payload, headers=headers, timeout=TIMEOUT
        )
        r.raise_for_status()
        j = r.json()
...
def _hf_generate(prompt: str, size: str, user: str) -> str | None:
    ...
    try:
        ...
        if r.status_code == 200 and r.headers.get("content-type", "").startswith(
            ...
            return _save_bytes(
                r.content, prompt, HF_T2I_MODEL, user, r.headers["content-type"]
            )
...
# ──────────────────────────────────────────────
def generate_image(
    prompt: str, size: str = DEFAULT_SIZE, user: str = "default"
) -> dict[str, Any]:
...
def edit_image(image_path: str, prompt: str, user: str = "default") -> dict[str, Any]:
    ...
    path = _a1111_img2img(image_path, prompt, user) or _openai_edit(
        image_path, prompt, user
    )
    if not path:
...
def feedback(
    image_path: str, rating: int, tags: list[str] | None = None
) -> dict[str, Any]:
        ...
        for _ in range(max(10, TIMEOUT // 2)):
            ...
            if jj.get("status") == "succeeded":
                ...
                if out:
                    path = _save_image_url(
                        out[0],
                        f"REMOVED_BG:{os.path.basename(image_path)}",
                        model,
                        user,
                    )
                    return {"ok": True, "path": path}
...
# ── FastAPI Router (defined at end to have all configs available)
router = APIRouter(prefix="/images_client", tags=["images_client"])

# Przykładowe endpointy API
@router.post("/generate")
def api_generate_image(prompt: str, size: str = None, user: str = "default"):
    result = generate_image(prompt, size or DEFAULT_SIZE, user)
    if not result.get("ok"):
        raise HTTPException(status_code=400, detail=result.get("error", "generate failed"))
    return result

@router.post("/edit")
def api_edit_image(image_path: str, prompt: str, user: str = "default"):
    result = edit_image(image_path, prompt, user)
    if not result.get("ok"):
        raise HTTPException(status_code=400, detail=result.get("error", "edit failed"))
    return result

@router.get("/analyze/{image_path}")
def api_analyze_image(image_path: str):
...
@router.get("/list")
def api_list_images(user: str = "default"):
    return list_images(user)
