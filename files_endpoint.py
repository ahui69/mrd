...
# Config
UPLOAD_DIR = os.getenv("UPLOAD_DIR", "/workspace/mrd69/out/uploads")
    ...
    for fname in os.listdir(UPLOAD_DIR):
        if fname.startswith(body.file_id):
            ...
            if os.path.exists(file_path):
                ...
                if body.analyze_content and analysis.get("extracted_text"):
                    try:
                        import monolit as M
...
async def delete_file(file_id: str, _=Depends(_auth)):
    """
    🗑️ Delete file
    """
    # Find and delete
